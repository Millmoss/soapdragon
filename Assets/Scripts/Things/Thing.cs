...
public class Thing	//I wanted to call it object but obviously that's not a good idea
{
	...
	private HashSet<string> useKeys;            //all use keywords for what this object is used for (food, cook, store, damage, heal, etc)
	private HashSet<string> constraintKeys;     //all restricting keywords for this object (stationary, indestructable, etc(?))
	...
	public Thing(string nm, Vector2Int pos, int wt, int dur, int nut, float eff, int utm, HashSet<string> ukey, HashSet<string> ckey, HashSet<string> fkey)
	{
		name = nm;
		position = pos;
		weight = wt;
	...
	{
		get { return new HashSet<string>(useKeys); }
	}
	...
	{
		get { return new HashSet<string>(constraintKeys); }
	}
